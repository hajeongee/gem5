...
SplitCPUAdapter::SplitCPUAdapter(const SplitCPUAdapterParams &params)
    : SimObject(params),
      simbricks::base::GenericBaseAdapter<SplitProtoM2C,
                                          SplitProtoC2M>::Interface(*this),
...
Port &SplitCPUAdapter::getPort(const std::string &if_name, PortID idx) {
...
void SplitCPUAdapter::CPUSidePort::recvRespRetry() {
  DPRINTF(SplitCPUAdapter, "recvRespRetry\n");
}

AddrRangeList SplitCPUAdapter::getAddrRanges() const {
...
void SplitCPUAdapter::sendRangeChange() {
  cpu_side.sendRangeChange();
}

bool SplitCPUAdapter::handleRequest(PacketPtr pkt) {
...
bool SplitCPUAdapter::handleResponse(PacketPtr pkt) {
...
bool SplitCPUAdapter::CPUSidePort::recvTimingReq(PacketPtr pkt) {
  owner->reqCount++;
  pkt->req->_sbReqSeq = owner->reqCount;
  owner->in_flight.insert({owner->reqCount, pkt});

  DPRINTF(SplitCPUAdapter,
          "insert reqCount: %d requestorID: %d"
          " taskId: %u\n pktptr: %p ReqPtr: %p\n",
          owner->reqCount, pkt->req->_requestorId, pkt->req->_taskId, pkt,
          pkt->req);
...
volatile union SplitProtoC2M *SplitCPUAdapter::c2mAlloc(bool syncAlloc,
                                                        bool functional) {
...
AddrRangeList SplitCPUAdapter::IntReqProxyPort::getAddrRanges() const {
  return owner->getAddrRanges();
}

void SplitCPUAdapter::MemSidePort::recvRangeChange() {
  ...
  DPRINTF(SplitCPUAdapter,
          "MemSidePort receive range change\n"
          "%d: %s\n",
...
void SplitCPUAdapter::IntRespProxyPort::recvRangeChange() {
  ...
  DPRINTF(SplitCPUAdapter, "IntRespProxyPort receive range change\n%d: %s\n",
          ranges.size(), ranges.front().to_string());
  ...
  owner->adapter.outSend((volatile SplitProtoC2M *)amsg,
                         SPLIT_PROTO_C2M_RECV | SPLIT_PROTO_C2M_OWN_MEM);
}
...
void SplitCPUAdapter::PioProxyPort::recvRangeChange() {
  ...
  DPRINTF(SplitCPUAdapter,
          "PioProxyPort receive range change\n"
          "%d: %s\n",
          ranges.size(), ranges.front().to_string());
...
void SplitCPUAdapter::CPUSidePort::recvFunctional(PacketPtr pkt) {
  return owner->handleFunctional(pkt);
}

size_t SplitCPUAdapter::introOutPrepare(void *data, size_t maxlen) {
...
void SplitCPUAdapter::introInReceived(const void *data, size_t len) {
...
void SplitCPUAdapter::handleInMsg(volatile SplitProtoM2C *msg) {
  ...
  switch (pkt_ty) {
    ...
    case SPLIT_PROTO_M2C_RECV:
      ...
      if (pkt_ty == (PACKET_ADDR_RANGE | PACKET_FUNCTIONAL | INT_REQ_PROXY)) {
        volatile struct SplitGem5AddrRange *amsg = &msg->addr_range;
        size = amsg->size;
        int i;
        for (i = 0; i < size; i++) {
        ...
        if (((enum Command)spkt->cmd == ReadResp) |
          ...
          if (search != in_flight.end()) {
            ...
            DPRINTF(SplitCPUAdapter, "no taskId found taskId: %u pktptr: %p\n",
                    (uint32_t)spkt->req._taskId, search->second);
...
void SplitCPUAdapter::handleFunctional(PacketPtr pkt) {
  ...
  spkt->req._requestorId = pkt->req->_requestorId;
  spkt->req._flags = pkt->req->_flags;
  spkt->req._cacheCoherenceFlags = pkt->req->_cacheCoherenceFlags;
  ...
  adapter.outSend((volatile SplitProtoC2M *)spkt,
                  SPLIT_PROTO_C2M_RECV | SPLIT_PROTO_C2M_OWN_MEM);
}

void SplitCPUAdapter::PktToMsg(PacketPtr pkt,
                              volatile union SplitProtoC2M *msg,
                              uint8_t pkt_type) {
  volatile struct SplitGem5Packet *spkt = &msg->packet;
