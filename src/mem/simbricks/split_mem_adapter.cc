...
SplitMEMAdapter::SplitMEMAdapter(const SplitMEMAdapterParams &params)
    : SimObject(params),
      simbricks::base::GenericBaseAdapter<SplitProtoC2M,
                                          SplitProtoM2C>::Interface(*this),
...
size_t SplitMEMAdapter::introOutPrepare(void *data, size_t maxlen) {
...
void SplitMEMAdapter::introInReceived(const void *data, size_t len) {
  assert(len == sizeof(struct SimbricksProtoMemIntro));
}

Port &SplitMEMAdapter::getPort(const std::string &if_name, PortID idx) {
...
void SplitMEMAdapter::IntReqProxyPort::recvRangeChange() {
  AddrRangeList ranges = getAddrRanges();
  int rsize = ranges.size();

  DPRINTF(SplitMEMAdapter, "IntReqProxyPort receive range change\n%d\n",
          ranges.size());

  if (rsize > 150) {
    panic("IntReqProxyPort: more than 150 address ranges: %d\n", \
      ranges.size());
  }
  ...
  owner->adapter.outSend((volatile SplitProtoM2C *)amsg,
                         SPLIT_PROTO_M2C_RECV | SPLIT_PROTO_M2C_OWN_CPU);
}

AddrRangeList SplitMEMAdapter::PioProxyPort::getAddrRanges() const {
  // return owner->getAddrRanges();
  return ranges_;
}
AddrRangeList SplitMEMAdapter::IntRespProxyPort::getAddrRanges() const {
  // return owner->getAddrRanges();
  return ranges_;
}

AddrRangeList SplitMEMAdapter::getAddrRanges() const {
  DPRINTF(SplitMEMAdapter, "getAddrRanges\n");
  return mem_side.getAddrRanges();
  // todo
}

void SplitMEMAdapter::handleFunctional(PacketPtr pkt) {
  DPRINTF(SplitMEMAdapter, "handlefunctional\n");
}

void SplitMEMAdapter::sendRangeChange() {
  DPRINTF(SplitMEMAdapter, "sending range change\n");
}

bool SplitMEMAdapter::MEMSidePort::recvTimingResp(PacketPtr pkt) {
  return owner->handleResponse(pkt);
}

void SplitMEMAdapter::MEMSidePort::recvRangeChange() {
...
bool SplitMEMAdapter::handleResponse(PacketPtr pkt) {
  ...
  adapter.outSend((volatile union SplitProtoM2C *)msg,
                  (SPLIT_PROTO_M2C_RECV | SPLIT_PROTO_M2C_OWN_CPU));

  return true;
}

void SplitMEMAdapter::handleInMsg(volatile SplitProtoC2M *msg) {
  uint8_t ty;
  ty = msg->dummy.own_type & SPLIT_PROTO_C2M_MSG_MASK;

  switch (ty) {
    case SPLIT_PROTO_C2M_SYNC:
      DPRINTF(SplitMEMAdapter, "received sync from CPU Side ts: %lu\n",
    ...
    case SPLIT_PROTO_C2M_RECV:
      if (msg->dummy.pkt_type & PACKET_ADDR_RANGE) {
        ...
        if (amsg->pkt_type & PIO_PROXY) {
          ...
          DPRINTF(SplitMEMAdapter, "received a PIO address range packet\n");
        ...
        req->_cacheCoherenceFlags = spkt->req._cacheCoherenceFlags;
